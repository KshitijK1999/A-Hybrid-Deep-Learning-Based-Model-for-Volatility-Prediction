...
#iterate through all the volatility series manually
...
def future_lstm_model():
    model_lstm = Sequential()
    model_lstm.add(LSTM(40, input_shape=(1, 1), activation='relu', kernel_initializer='lecun_uniform', return_sequences=False))
    model_lstm.add(Dropout(0.05))
    model_lstm.add(Flatten())
...
for sym in symbols:
  ...
  df1['forecast_vol_Copper'] = ((data2['x'].values)) 
  ...
  for win_sz in window_sizes:
    ...
    for i in range(20):
      ...
      history_model_GARCH_lstm = model.fit([X_tr_t, X_train1], y_train, epochs=100, batch_size=32, verbose=1, shuffle=False,callbacks=[early_stop])
      #train_acc, test_acc = test_model(model, X_tr_t, X_tst_t, y_train, y_test)
      y_pred_test_GARCH_LSTM = model.predict([X_tst_t, X_test1])
      y_pred_test_GARCH_LSTM_train = model.predict([X_tr_t, X_train1])
    ...
    for i in range(len(plot_GARCH_lstm)):
        plot_GARCH_lstm[i] = plot_GARCH_lstm[i] / 20
    ...
    for i in range(len(plot_GARCH_lstm_train)):
        plot_GARCH_lstm_train[i] = plot_GARCH_lstm_train[i] / 20
