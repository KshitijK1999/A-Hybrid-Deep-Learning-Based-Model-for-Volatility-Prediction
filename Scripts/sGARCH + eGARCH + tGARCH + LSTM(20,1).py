...
data1 = pd.read_csv('eGarch1110Copper_252roll.csv')
data2 = pd.read_csv('eGarch1110Copper_252roll.csv')
data3 = pd.read_csv('sGarch1110Copper_252roll.csv')
data4 = pd.read_csv('sGarch1110Copper_252roll.csv')
data5 = pd.read_csv('tGarch1210Copper_252roll.csv')
data6 = pd.read_csv('tGarch1210Copper_252roll.csv')
...
data5['x'] = data5['x'].replace([0, -np.inf], data5['x'].nsmallest(2).iloc[-1])
data6['x'] = data6['x'].replace([0, -np.inf], data6['x'].nsmallest(2).iloc[-1])
...
data5['x'] = data5['x'].replace([np.inf], data5['x'].nlargest(2).iloc[-1])
data6['x'] = data6['x'].replace([np.inf], data6['x'].nlargest(2).iloc[-1])
...
data['stdev_Alluminium'] = data['LR_All'].rolling(window=22, center=False).var()
...
data['stdev_Zinc'] = data['LR_Zinc'].rolling(window=22, center=False).var()
...
data6.drop(0,axis=0,inplace=True)
...
data1['x'] = data1['x']*1000
data2['x'] = data2['x']*1000
data3['x'] = data3['x']*1000
data4['x'] = data4['x']*1000
data5['x'] = data5['x']*1000
data6['x'] = data6['x']*1000
...
def make_GARCH_lstm_model(window_size):
    ...
    model_lstm.add(LSTM(160, input_shape=(window_size, 1), activation='relu', kernel_initializer='lecun_uniform', return_sequences=False))
...
def future_lstm_model1():
    model_lstm = Sequential()
    model_lstm.add(LSTM(160, input_shape=(1, 1), activation='relu', kernel_initializer='lecun_uniform', return_sequences=False))
    model_lstm.add(Dropout(0.05))
    model_lstm.add(Flatten())
...
for sym in symbols:
  ...
  df3['forecast_vol_Copper_inp3'] = ((data6['x'].values))
  ...
  train_sc3 = sc.fit_transform(train3)
  test_sc3 = sc.transform(test3)
  ...
  for win_sz in window_sizes:
    ...
    for i in range(5):
      ...
      X_train3, y_train3 = window_transform1(train_sc3, win_sz)
      X_test3, y_test3 = window_transform1(test_sc3, win_sz)
    
      y_train3 = np.delete(y_train3,1,1)
      y_test3 = np.delete(y_test3,1,1)
      X_train3 = np.delete(X_train3,0,1)
      X_test3 = np.delete(X_test3,0,1)
      ...
      model = Model(inputs=[model1.input, model2.input,model3.input,model4.input], outputs=z)
      model.compile(loss='mean_squared_error', optimizer='adam')

      early_stop = EarlyStopping(monitor='loss', patience=6, verbose=1)
      history_model_GARCH_lstm = model.fit([X_tr_t, X_train1,X_train2,X_train3], y_train, epochs=100, batch_size=32, verbose=1, shuffle=False,callbacks=[early_stop])
      #train_acc, test_acc = test_model(model, X_tr_t, X_tst_t, y_train, y_test)
      y_pred_test_GARCH_LSTM = model.predict([X_tst_t, X_test1,X_test2,X_test3])
      y_pred_test_GARCH_LSTM_train = model.predict([X_tr_t, X_train1,X_train2,X_train3])
